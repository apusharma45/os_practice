...
int find_replacement_frame_mfu(int requested_page) {
    int most_frequent = 0;
    ...
    int current_page = RAM[most_frequent];
...
void handle_request(int requested_page) {
    if(valid[requested_page]) {
        ...
        int replaced_frame = find_replacement_frame_mfu(requested_page);
        cout << "Frame " << replaced_frame << " is replaced and " << requested_page << " is loaded" << endl;
        ++current_time;
        frequency[replaced_frame] = 1;
